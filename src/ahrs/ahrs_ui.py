"""User interface module for the AHRS dashboard."""
...
class AttitudeView(QWidget):
    ...
    def _rotate(self, point: tuple[float, float, float]) -> tuple[float, float, float]:
        x, y, z = point
        roll, pitch, yaw = map(math.radians, (self.roll, self.pitch, self.heading))
...
class HistoryPlot(QWidget):
    ...
    def add(self, roll: float, pitch: float, heading: float) -> None:
        for value, (history, _color) in zip((roll, pitch, heading), self.series.values(), strict=True):
